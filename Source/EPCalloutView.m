#import "EPCalloutView.h"
#import "EPCalloutDefaultContentView.h"

NSInteger const EPCalloutPermittedArrowDirectionAny = EPCalloutArrowDirectionTop | EPCalloutArrowDirectionLeft | EPCalloutArrowDirectionBottom | EPCalloutArrowDirectionRight;

static CGSize const EPCalloutViewDefaultArrowSize = {30,15};
static CGFloat const EPCalloutViewDefaultCornerRadius = 10;
static CGFloat const EPCalloutViewDefaultArrowOffset = 5.0;
static UIEdgeInsets const EPCalloutViewDefaultInternalInsets = {5,5,5,5};
static UIEdgeInsets const EPCalloutViewDefaultExternalInsets = {10,10,10,10};

@interface EPCalloutView ()
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _arrowOffset = EPCalloutViewDefaultArrowOffset;
        _arrowSize = EPCalloutViewDefaultArrowSize;
        _cornerRadius = EPCalloutViewDefaultCornerRadius;
        _internalInsets = EPCalloutViewDefaultInternalInsets;
        _externalInsets = EPCalloutViewDefaultExternalInsets;
        _shouldHighlightOnTouch = YES;
        _permittedArrowDirections = EPCalloutPermittedArrowDirectionAny;
...
- (void)updateConstraintsForArrowDirection:(EPCalloutArrowDirection)arrowDirection {
...
- (void)drawPointerForControlPoint:(CGPoint)controlPoint animated:(BOOL)animated {
    ...
    // Top edge arrow
    if (self.currentArrowDirection == EPCalloutArrowDirectionTop) {
    ...
    // Right edge arrow
    if (self.currentArrowDirection == EPCalloutArrowDirectionRight) {
    ...
    // Bottom edge arrow
    if (self.currentArrowDirection == EPCalloutArrowDirectionBottom) {
    ...
    // Left edge arrow
    if (self.currentArrowDirection == EPCalloutArrowDirectionLeft) {
...
- (void)setCurrentArrowDirection:(EPCalloutArrowDirection)currentArrowDirection {
...
- (void)presentFromRect:(CGRect)presentationRect inView:(UIView *)view withConstrainingView:(UIView *)constrainingView animated:(BOOL)animated {
    ...
    self.currentArrowDirection = EPCalloutArrowDirectionNone;
    ...
    if ([self isArrowDirectionPermitted:EPCalloutArrowDirectionBottom] && necessaryHeight < availableTopSpace && minimumTopBottomX <= midRectX && midRectX <= maximumTopBottomX) {
        self.currentArrowDirection = EPCalloutArrowDirectionBottom;
    ...
    } else if ([self isArrowDirectionPermitted:EPCalloutArrowDirectionTop] && necessaryHeight < availableBottomSpace && minimumTopBottomX <= midRectX && midRectX <= maximumTopBottomX) {
        self.currentArrowDirection = EPCalloutArrowDirectionTop;
        originX = CGRectGetMidX(presentationRect) - (CGRectGetWidth(self.bounds) / 2.0);
        originY = CGRectGetMaxY(presentationRect) + self.arrowOffset;
    } else if ([self isArrowDirectionPermitted:EPCalloutArrowDirectionLeft] && necessaryWidth < availableRightSpace && minimumLeftRightY <= midRectY && midRectY <= maximumLeftRightY) {
        self.currentArrowDirection = EPCalloutArrowDirectionLeft;
    ...
    } else if ([self isArrowDirectionPermitted:EPCalloutArrowDirectionRight] && necessaryWidth < availableLeftSpace && minimumLeftRightY <= midRectY && midRectY <= maximumLeftRightY) {
        self.currentArrowDirection = EPCalloutArrowDirectionRight;
    ...
    if (self.currentArrowDirection == EPCalloutArrowDirectionNone && self.permittedArrowDirections != EPCalloutArrowDirectionNone) {
        for (NSNumber *space in availableSpace) {
            CGFloat availableSpace = space.floatValue;
            if (availableSpace == availableTopSpace && [self isArrowDirectionPermitted:EPCalloutArrowDirectionBottom]) {
                self.currentArrowDirection = EPCalloutArrowDirectionBottom;
                ...
                originY = CGRectGetMinY(presentationRect) - self.arrowOffset - CGRectGetHeight(self.bounds);
                break;
            } else if (availableSpace == availableBottomSpace && [self isArrowDirectionPermitted:EPCalloutArrowDirectionTop]) {
                self.currentArrowDirection = EPCalloutArrowDirectionTop;
                if (presentationRectMiddleX <= constraintHalfWidth) {
                    originX = presentationRectMiddleX - [self minControlPointX];
                } else {
                ...
                originY = CGRectGetMaxY(presentationRect) + self.arrowOffset;
                break;
            } else if (availableSpace == availableRightSpace && [self isArrowDirectionPermitted:EPCalloutArrowDirectionLeft]) {
                self.currentArrowDirection = EPCalloutArrowDirectionLeft;
                originX = CGRectGetMaxX(presentationRect) + self.arrowOffset;
                ...
                break;
            } else if (availableSpace == availableLeftSpace && [self isArrowDirectionPermitted:EPCalloutArrowDirectionRight]) {
                self.currentArrowDirection = EPCalloutArrowDirectionRight;
                originX = CGRectGetMinX(presentationRect) - self.arrowOffset - CGRectGetWidth(self.bounds);
                if (presentationRectMiddleY <= constraintHalfHeight) {
                    originY = presentationRectMiddleY - [self minControlPointY];
    ...
    } else if (self.permittedArrowDirections == EPCalloutArrowDirectionNone) {
...
- (BOOL)isArrowDirectionPermitted:(EPCalloutArrowDirection)direction {
